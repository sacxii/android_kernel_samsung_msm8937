...
int ili_lcm_bias_power_init(struct ilitek_ts_data *idev)
{
	int ret;
	...
	if (IS_ERR(idev->lcm_ibb)){
		...
		ILI_ERR("Regulator get failed lcm_ibb ret=%d", ret);
...
int ili_lcm_bias_power_deinit(struct ilitek_ts_data *idev)
{
...
int ili_lcm_power_source_ctrl(struct ilitek_ts_data *idev, int enable)
{
	int rc;
	...
	if (idev->lcm_lab!= NULL && idev->lcm_ibb!= NULL){
		if (enable){
			if (atomic_inc_return(&(idev->lcm_lab_power)) == 1) {
				...
				if (rc) {
					...
					ILI_ERR("Regulator lcm_lab enable failed rc=%d", rc);
				}
			}
			else {
				atomic_dec(&(idev->lcm_lab_power));
			...
			if (atomic_inc_return(&(idev->lcm_ibb_power)) == 1) {
				...
				if (rc) {
					...
					ILI_ERR("Regulator lcm_ibb enable failed rc=%d", rc);
				}
			}
			else {
				atomic_dec(&(idev->lcm_ibb_power));
			...
			if (atomic_dec_return(&(idev->lcm_lab_power)) == 0) {
				...
				if (rc)
				{
					...
					ILI_ERR("Regulator lcm_lab disable failed rc=%d", rc);
				}
			}
			else{
				atomic_inc(&(idev->lcm_lab_power));
			...
			if (atomic_dec_return(&(idev->lcm_ibb_power)) == 0) {
				...
				if (rc)	{
					...
					ILI_ERR("Regulator lcm_ibb disable failed rc=%d", rc);
				}
			}
			else{
				atomic_inc(&(idev->lcm_ibb_power));
		...
		ILI_ERR("Regulator lcm_ibb or lcm_lab is invalid");
	return 0;
}
...
static int ilitek_get_tp_module(void)
{
	...
	if (NULL == chosen)
		ILI_ERR("DT: chosen node is not found\n");
	else {
		of_property_read_string(chosen, "bootargs", &panel_name);
		ILI_INFO("cmdline:%s\n", panel_name);
	}
	if (NULL != strstr(panel_name, "mdss_dsi_liansi_panda_ili9882h_720p_video")){
		fw_num = MODEL_LS_PANDA;
	} else  if (NULL != strstr(panel_name, "mdss_dsi_liansi_panda_lrpol_ili9882h_720p_video")){
		fw_num = MODEL_LS_PANDA_LRPOL;
	} else if (NULL != strstr(panel_name, "mdss_dsi_chuangwei_panda_ili9882h_720p_video")){
		fw_num = MODEL_SKW_PANDA;
	} else  if (NULL != strstr(panel_name, "mdss_dsi_chuangwei_panda_lrpol_ili9882h_720p_video")){
		fw_num = MODEL_SKW_PANDA_LRPOL;
	} else  if (NULL != strstr(panel_name, "mdss_dsi_liansi_mdt_ili9882n_720p_video")){
		fw_num = MODEL_LS_MDT;
...
static void ili_update_tp_module_info(void)
{
	...
	case MODEL_SKW_PANDA_LRPOL:
		...
		ilits->md_fw_filp_path = SKW_PANDA_LRPOL_FW_FILP_PATH;
		ilits->md_fw_rq_path = SKW_PANDA_LRPOL_FW_REQUEST_PATH;
		ilits->md_ini_path = SKW_PANDA_LRPOL_INI_NAME_PATH;
		ilits->md_ini_rq_path = SKW_PANDA_LRPOL_INI_REQUEST_PATH;
		ilits->md_fw_ili = CTPM_FW_SKW_PANDA_LRPOL;
		ilits->md_fw_ili_size = sizeof(CTPM_FW_SKW_PANDA_LRPOL);
		break;
	/*HS50 code for HS50-1707 by gaozhengwei at 2020/09/14 end*/
	/*HS50 code for SR-QL3095-01-74 by wangdeyan at 2020/07/25 start*/
	case MODEL_SKW_PANDA:
		ilits->md_name = "SKW_PANDA";
		ilits->md_fw_filp_path = SKW_PANDA_FW_FILP_PATH;
		ilits->md_fw_rq_path = SKW_PANDA_FW_REQUEST_PATH;
		ilits->md_ini_path = SKW_PANDA_INI_NAME_PATH;
		ilits->md_ini_rq_path = SKW_PANDA_INI_REQUEST_PATH;
		ilits->md_fw_ili = CTPM_FW_SKW_PANDA;
		ilits->md_fw_ili_size = sizeof(CTPM_FW_SKW_PANDA);
		break;
	/*HS50 code for HS50-1707 by gaozhengwei at 2020/09/14 start*/
	case MODEL_LS_PANDA_LRPOL:
		ilits->md_name = "LS_PANDA_LRPOL";
		ilits->md_fw_filp_path = LS_PANDA_LRPOL_FW_FILP_PATH;
		ilits->md_fw_rq_path = LS_PANDA_LRPOL_FW_REQUEST_PATH;
		ilits->md_ini_path = LS_PANDA_LRPOL_INI_NAME_PATH;
		ilits->md_ini_rq_path = LS_PANDA_LRPOL_INI_REQUEST_PATH;
		ilits->md_fw_ili = CTPM_FW_LS_PANDA_LRPOL;
		ilits->md_fw_ili_size = sizeof(CTPM_FW_LS_PANDA_LRPOL);
		break;
	/*HS50 code for HS50-1707 by gaozhengwei at 2020/09/14 end*/
	case MODEL_LS_PANDA:
	...
	case MODEL_LS_MDT:
		...
		ilits->md_ini_rq_path = LS_MDT_INI_REQUEST_PATH;
		ilits->md_fw_ili = CTPM_FW_LS_MDT;
		ilits->md_fw_ili_size = sizeof(CTPM_FW_LS_MDT);
		break;
	/*HS50 code for SR-QL3095-01-74 by wangdeyan at 2020/07/25 end*/
	/*HS50 code for SR-QL3095-01-723 by fengzhigang at 2020/09/03 start*/
	case MODEL_TXD_INX_ILI9882N:
		ilits->md_name = "TXD_INX_ILI9882N";
		ilits->md_fw_filp_path = TXD_INX_ILI9882N_FW_FILP_PATH;
		ilits->md_fw_rq_path = TXD_INX_ILI9882N_FW_REQUEST_PATH;
		ilits->md_ini_path = TXD_INX_ILI9882N_INI_NAME_PATH;
		ilits->md_ini_rq_path = TXD_INX_ILI9882N_INI_REQUEST_PATH;
