...
#if FTS_TEST_EN
extern int qc_ito_TestResult_fts;
extern int qc_fts_test_entry(char *ini_file_name);
...
/* HS70 add for HS70-193 add tsp cmd feature and add gesture aot_enable by gaozhengwei at 2019/10/24 start */
#if HQ_FTS_POWER_SOURCE_CUST_EN

static int fts_lcm_bias_power_init(struct fts_ts_data *data)
{
	int ret;
	...
	if (IS_ERR(data->lcm_ibb)){
		...
		FTS_ERROR("Regulator get failed lcm_ibb ret=%d", ret);
...
static int fts_lcm_bias_power_deinit(struct fts_ts_data *data)
{
...
static int fts_lcm_power_source_ctrl(struct fts_ts_data *data, int enable)
{
	int rc;
	...
	if (data->lcm_lab!= NULL && data->lcm_ibb!= NULL){
		if (enable){
			if (atomic_inc_return(&(data->lcm_lab_power)) == 1) {
				...
				if (rc) {
					...
					FTS_ERROR("Regulator lcm_lab enable failed rc=%d", rc);
				}
			}
			else {
				atomic_dec(&(data->lcm_lab_power));
			...
			if (atomic_inc_return(&(data->lcm_ibb_power)) == 1) {
				...
				if (rc) {
					...
					FTS_ERROR("Regulator lcm_ibb enable failed rc=%d", rc);
				}
			}
			else {
				atomic_dec(&(data->lcm_ibb_power));
			...
			if (atomic_dec_return(&(data->lcm_lab_power)) == 0) {
				...
				if (rc)
				{
					...
					FTS_ERROR("Regulator lcm_lab disable failed rc=%d", rc);
				}
			}
			else{
				atomic_inc(&(data->lcm_lab_power));
			...
			if (atomic_dec_return(&(data->lcm_ibb_power)) == 0) {
				...
				if (rc)	{
					...
					FTS_ERROR("Regulator lcm_ibb disable failed rc=%d", rc);
				}
			}
			else{
				atomic_inc(&(data->lcm_ibb_power));
		...
		FTS_ERROR("Regulator lcm_ibb or lcm_lab is invalid");
	return 0;
}
...
/* Huaqin add for HS70-150 HardWareInfo by gaozhengwei at 2019/10/11 start */
...
static ssize_t fts_proc_getinfo_read(struct file *filp, char __user *buff, size_t size, loff_t *pPos)
{
	...
	if (ret >= 0 && fwupgrade != NULL && fwupgrade->module_info != NULL) {
        snprintf(buf, 150, "IC=FT8615-AB module=%s fw_ver=%x\n",fwupgrade->module_info->vendor_name,fwver);
	} else {
		snprintf(buf, 150, "IC=FT8615-AB module=NULL fw_ver=NA\n");
	}
...
#endif
/* Huaqin add for HS70-150 HardWareInfo by gaozhengwei at 2019/10/11 end */
...
#endif
/* HS70 add for HS70-159 by gaozhengwei at 2019/11/06 start */
        ret = fts_enter_test_environment(1);
        if (ret < 0) {
            FTS_ERROR("enter test environment fail");
        } else {
            qc_fts_test_entry(fwname);//ito test
        }
        ret = fts_enter_test_environment(0);
        if (ret < 0) {
            FTS_ERROR("enter normal environment fail");
        }
/* HS70 add for HS70-159 by gaozhengwei at 2019/11/06 end */
#if defined(FTS_ESDCHECK_EN) && (FTS_ESDCHECK_EN)
	...
	count = sprintf(buf, "%d\n", qc_ito_TestResult_fts);//return test  result
	return count;
}
...
static int fts_input_report_b(struct fts_ts_data *data)
{
    ...
    for (i = 0; i < data->touch_point; i++) {
        ...
        if (EVENT_DOWN(events[i].flag)) {
            ...
            /* HS70 add for HS70-1883 by gaozhengwei at 2019/12/05 start */
            ...
            #else
                FTS_DEBUG("[A]P%d [p:%d,tm:%d] DOWN!",
                    events[i].id, events[i].p, events[i].area);
            #endif
            /* HS70 add for HS70-1883 by gaozhengwei at 2019/12/05 end */
            }
        } else {
            uppoint++;
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
    ...
    if (NULL == chosen)
        FTS_ERROR("DT: chosen node is not found\n");
    else {
        of_property_read_string(chosen, "bootargs", &pdata->panel_name);
        FTS_INFO("cmdline:%s\n", pdata->panel_name);
    }
    /* HS70 add for HS70-1119 by gaozhengwei at 2019/11/14 end */
    FTS_FUNC_EXIT();
...
static int fts_charger_notifier_callback(struct notifier_block *nb,unsigned long val, void *v)
{
    ...
    if(fwupgrade->ts_data->fw_loaded == 1){
        psy= power_supply_get_by_name("usb");	//baterry
        if (!psy) {
        ...
        if (!strcmp(psy->desc->name, "usb")) {
            if (psy && ts && val == POWER_SUPPLY_PROP_STATUS) {
                ret = power_supply_get_property(psy, POWER_SUPPLY_PROP_PRESENT,&prop);//power_supply_get_property
                if (ret < 0) {
...
/* HS70 add for HS70-193 add tsp cmd feature and add gesture aot_enable by gaozhengwei at 2019/10/24 start */
#if HQ_FTS_POWER_SOURCE_CUST_EN
	atomic_set(&(ts_data->lcm_lab_power), 0);
	atomic_set(&(ts_data->lcm_ibb_power), 0);
	ret = fts_lcm_bias_power_init(ts_data);

	if (ret) {
		FTS_ERROR("power resource init error!\n");
		goto err_power_resource_init_fail;
	}

	fts_lcm_power_source_ctrl(ts_data, 1);
#endif
/* HS70 add for HS70-193 add tsp cmd feature and add gesture aot_enable by gaozhengwei at 2019/10/24 end */
...
/* Huaqin add for HS70-142 ito test by gaozhengwei at 2019/10/08 end */
    ...
    if (ret) {
        /* HS70 add for HS70-1379 by gaozhengwei at 2019/11/24 start */
        FTS_ERROR("init glove/cover/charger/high_sensitivity fail");
        /* HS70 add for HS70-1379 by gaozhengwei at 2019/11/24 end */
    }
...
/* HS70 add for HS70-193 add tsp cmd feature and add gesture aot_enable by gaozhengwei at 2019/10/24 start */
#if HQ_FTS_POWER_SOURCE_CUST_EN
	fts_lcm_power_source_ctrl(ts_data, 0);
	fts_lcm_bias_power_deinit(ts_data);
...
/* HS70 add for HS70-193 add tsp cmd feature and add gesture aot_enable by gaozhengwei at 2019/10/24 start */
#if HQ_FTS_POWER_SOURCE_CUST_EN
        fts_lcm_power_source_ctrl(ts_data, 0);//disable vsp/vsn
...
/* HS70 add for HS70-193 add tsp cmd feature and add gesture aot_enable by gaozhengwei at 2019/10/24 start */
#if HQ_FTS_POWER_SOURCE_CUST_EN
            fts_lcm_power_source_ctrl(ts_data, 1);//enable vsp/vsn
            FTS_INFO("fts_lcm_power_source_ctrl enable vsp/vsn\n");
#endif
/* HS70 add for HS70-193 add tsp cmd feature and add gesture aot_enable by gaozhengwei at 2019/10/24 end */
    }
...
static int fts_ts_probe(struct spi_device *spi)
{
    ...
    /* huaqin add for HS70-124 bringup TP function by gaozhengwei at 2019/10/03 start */
    spi->chip_select = 0;//only 0
    /* huaqin add for HS70-124 bringup TP function by gaozhengwei at 2019/10/03 end */
...
/* Huaqin add for HS70-195 add cmd for get IC and binary FW version by gaozhengwei at 2019/10/15 start */
    ret = sec_cmd_init(&ts_data->sec, fts_commands, fts_get_array_size(), SEC_CLASS_DEVT_TSP);
    if (ret < 0) {
...
/* HS70 add for HS70-735 by liufurong at 2019/11/05 start */
